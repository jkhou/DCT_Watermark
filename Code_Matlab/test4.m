...
clc;
%-----------------------读入图像-------------------------------------%
markbefore=imread('shuiyin999.bmp');%水印图片
...
cover_image=imread('yuantu2.bmp');%carrier_image载体图片
%cover_image=imread('shuiyin2.bmp');
cover_image=rgb2gray(cover_image);
subplot(2,3,2),imshow(cover_image,[]),title('载体图像'); %[]表示显示时灰度范围为image上的灰度最小值到最大值
imwrite(cover_image,'载体图像.bmp','bmp'); 
...
result = uint8(result);
imwrite(result,'markresule2.bmp','bmp');      %存储添加水印后的图像
...
switch choice        %读入输入的选择  withmark为等待提取水印的图像
    case 1
        ...
        imwrite(withmark,'加入高斯白噪声后的图像.bmp','bmp');      %存储添加水印后的图像
    case 2
         result_2=result;
         ...
         title('高斯低通滤波后图像'); 
         withmark=result_2;
         imwrite(result_2,'高斯低通滤波后图像.bmp','bmp');
    case 3
        result_3=result;
        ...
        title('上方剪切后图像');
        withmark=result_3;
        imwrite(result_3,'上方剪切后图像.bmp','bmp');
    case 4
        ...
        title('旋转10度后图像');
        withmark=result_4;
        imwrite(result_4,'经JPEG压缩后图像.bmp','bmp');
    case 5
        result_5 = result; 
        ...
        title('经JPEG压缩后图像'); 
        withmark=WImage5cl;
        imwrite(WImage5cl,'经JPEG压缩后图像.bmp','bmp');
    case 6
        result_6=result;
        ...
        imwrite(withmark,'加入椒盐噪声后的图像.bmp','bmp');
    case 7
        subplot(2,3,4);
        imshow(result,[]);
        title('未受攻击的水印图像');
